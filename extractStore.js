...
const storeBlockRegex = /\/\/ --- STORE & UPLOADED REWARDS ---.*?(?=\/\/ --- MAP CANVAS SYSTEM ---)/s;
...
if (match) {
    ...
    if(!newAppJs.includes('export { getSafeImageSrc }')) {
        newAppJs += `\nexport { appState, buyItem };\n`; // Export things store needs, wait state.js has appState
    }
